class Solution:
    ...
    def minPathSum(self, A):
    	...
    	return self.Pathsum(0, 0, n, m, A, self.visited, self.dp)
    def Pathsum(self, i, j ,n, m, arr, visited, dp):
    	if visited[i][j] == 1:
    		return dp[i][j]
    	visited[i][j] = 1
    	if i == n-1 and j == m-1:
    		...
    		dp[i][j] = arr[i][j]+self.Pathsum(i, j+1, n, m, arr, visited, dp)
    	elif j == m-1:
    		#return arr[i][j]+self.Pathsum(i+1,j, n, m, arr, visited, dp)
    		dp[i][j] = arr[i][j]+self.Pathsum(i+1,j, n, m, arr, visited, dp)
    	else:
    		#return arr[i][j]+min(self.Pathsum(i+1, j, n, m, arr, visited, dp), self.Pathsum(i, j+1, n, m, arr, visited, dp))
    		dp[i][j] = arr[i][j]+max(self.Pathsum(i+1, j, n, m, arr, visited, dp), self.Pathsum(i, j+1, n, m, arr, visited, dp))
    	return dp[i][j]
sol = Solution()
#print(sol.minPathsum([[1,2,3],[4,5,6],[7,8,9]]))
